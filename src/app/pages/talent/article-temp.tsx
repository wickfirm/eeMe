...
import {getArticleIndexData, getArticleTempIndexData, getMoreTalentsArticles} from "../../core/talent/requests";
...
const TalentArticleTempPage: FC = () => {
    ...
    useEffect(() => {
        ...
        getArticleTempIndexData(params.talent , params.article ,currentLanguageCode).then(response => {
        ...
        [ t ,params.talent , params.article , renderedTalentsPriority , talentsPriorityPageNumber , articlesTotal , prevPagePriority, currPagePriority, lastPagePriority])
                        ...
                        <div className="row justify-content-between pt-20">
                            <div className={`${talent?.talent_order_types.length > 0 ? 'col-md-7' : 'col-md-10'}`}>
                                <div>
                                            ...
                                            <ul >
                                                <li >
                                                    ...
                                                    <Link className="text-color text-capitalize"
                                                       to={currentLanguageCode === 'en' ? `/en/talent-temp/${talent?.slug?.en}` :  `/ar/talent-temp/${talent?.slug?.ar}` }>
                                    ...
                                    <div className="share-block pt-20">
                                    ...
                                    <div className="col-md-4">
                                        ...
                                        <div className="pt-20">
                                            ...
                                            <Link to={currentLanguageCode === 'en' ? '/en/talent-temp/' + talent.slug.en + '/book' : '/ar/talent/' + talent.slug.ar + '/book'} className="btn btn-primary w-100">{t('book')}</Link>
                                        </div>
                                    </div>
                                        ) : (null )}
                            ...
                            <div className="row">
                                    ...
                                    <div className="articles">
                                        <div className="container">
                                            {articles && articles?.slice(0, 1).map((article: any) => (
                                                <HorizontalArticle article={article}  key={article.id} talent={talent}  link={'talent-temp'}/>
                                            ))}
                                            <div className='row pt-20'>
                                                {articles && articles?.slice(1, 4).map((article: any) => (
                                                    <VerticalArticle article={article}  key={article.id} talent={talent}  link={'talent-temp'}/>
                                                ))}
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div className="col-md-4">
                                    {articles && articles?.slice(4, 6).map((article: any) => (
                                        <AsideArticle article={article}  key={article.id}  talent={talent}  link={'talent-temp'}/>
                                    ))}
...
export default TalentArticleTempPage;
