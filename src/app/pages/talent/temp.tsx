...
const TalentTempPage: FC = () => {
    ...
    useEffect(() => {
        ...
        getTalentTempData(params.talent, currentLanguageCode).then(response => {
         ...
         setTimeout(() => {
             getInTheSpotFirstItem (params.talent, currentLanguageCode, 0).then(response => {
    ...
    return (
        <Fragment >
            ...
            <SocialMeta title ={currentLanguageCode === 'en' && talent && talent.user && talent.user.name  ? talent.user.name.en + " | " + t('webName') : currentLanguageCode === 'ar' && talent && talent.user && talent.user.name  ? talent.user.name.ar + " | " + t('webName')  : t('socialMeta.home.title')}
                        ...
                        link={ `/${currentLanguageCode}/talent-temp/${params.talent}`}
                        index={false}

            />
            <Header page={TALENT_PAGE_ID} href={currentLanguageCode === 'en' && talent ? '/ar/talent-temp/' + talent?.slug.ar : talent ? '/en/talent-temp/'+ talent?.slug.en : ""} />
            <section className="" >
               ...
               {talent && talent.is_verified ? (
                   <i className="far fa-check-circle bg-primary text-white bdr-50"></i>) : (null)}
                                    ...
                                    <div className="talent-social-links pt-20">
                                        <ul>
                                            {talent?.talent_social?.length > 0 && talent?.talent_social.map((talent_social: any) => (
                                    ...
                                    {talent?.is_available && talent?.is_verified && talent?.talent_order_types.length > 0 ? (
                                        <div className="pt-25 pb-25">
                                            <div className="">
                                                <Link to={`${currentLanguageCode === 'en' ? `/en/talent-temp/${talent.slug.en}/book` :  `/ar/talent-temp/${talent.slug.ar}/book`}`}
                ...
                <Articles articles={articles} total={articlesTotal} talent={talent}  link={'talent-temp'}/>
